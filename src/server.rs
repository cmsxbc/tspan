  ...
  function buildBar(val, maxVal, baseVal, isAllTime) {
    ...
    if (isAllTime) {
      return '<div style="flex:1;display:flex;align-items:center;">' +
             '<div style="width:' + barPct.toFixed(1) + '%;height:14px;background:#d4a017;border-radius:4px;"></div></div>';
    } else if (val <= baseVal) {
      ...
      return '<div style="flex:1;height:14px;background:#ebedf0;border-radius:4px;position:relative;">' +
             '<div style="position:absolute;left:0;top:0;width:' + basePct.toFixed(1) + '%;height:100%;background:#0969da;border-radius:4px 0 0 4px;"></div>' +
             '<div style="position:absolute;left:' + basePct.toFixed(1) + '%;top:0;width:' + (barPct - basePct).toFixed(1) + '%;height:100%;background:#2da44e;border-radius:0 4px 4px 0;"></div>' +
             '</div>';
    }
  }
  let html = '';
