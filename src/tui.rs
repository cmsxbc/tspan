use crate::{
    db::{self, ApiToken, DbPool, Record},
...
impl App {
    fn new(pool: DbPool, options: TuiOptions) -> Result<Self> {
        ...
        let mut app = Self {
            pool,
            timezone,
            command_token_limit: options.command_token_limit,
            view: View::Overview,
    ...
    fn refresh_client_ids(&mut self, preferred: Option<&str>) -> Result<()> {
        ...
        let mut clients = {
            ...
            let mut clients =
                db::distinct_client_ids(&mut conn).context("could not load clients")?;
            clients.extend(
                db::list_api_tokens(&mut conn)
                    .context("could not load token clients")?
                    .into_iter()
    ...
    fn refresh_all(&mut self) -> Result<()> {
        ...
        let (overview, records, records_total, active, tokens) = {
            let mut conn = self.pool.lock();
            let overview = OverviewData {
                stats: stats::compute_stats(&mut conn, &client_id, "", "", &self.timezone)
                    .context("could not compute summary statistics")?,
                streaks: stats::compute_streaks(&mut conn, &client_id, "", "", &self.timezone)
                    .context("could not compute streak statistics")?,
                distribution: stats::compute_session_distribution(&mut conn, &client_id, "", "")
                    .context("could not compute session distribution")?,
                by_client: stats::compute_stats_by_client(&mut conn, &client_id)
                    .context("could not compute client statistics")?,
                by_alias: stats::compute_stats_by_alias(&mut conn, &client_id)
                    .context("could not compute alias statistics")?,
                by_command: stats::compute_stats_by_command(
                    &mut conn,
                    &client_id,
                    1,
                    self.command_token_limit,
                )
                .context("could not compute command statistics")?,
            };
            let (mut records, total) = db::list_records_page(
                &mut conn,
                &client_id,
                "",
                "",
            ...
            .context("could not load records")?;
            let pages = total_pages(total, self.page_size);
            if self.records_page > pages {
                self.records_page = pages;
                records = db::list_records_page(
                    &mut conn,
                    &client_id,
                    "",
                    "",
            ...
            let mut active = db::get_orphaned_sessions_admin(&mut conn)
                .context("could not load active sessions")?;
            let mut tokens = db::list_api_tokens(&mut conn).context("could not load API tokens")?;
            if client_id != GLOBAL_CLIENT {
                active.retain(|record| record.client_id == client_id);
                tokens.retain(|token| token.client_id == client_id);
            }
            (overview, records, total, active, tokens)
        };
    ...
    fn selected_active(&self) -> Option<&Record> {
        self.active_state
            .selected()
        ...
        let result: Result<String> = (|| {
            ...
            let message = match action {
                AdminAction::DeleteRecord(id) => {
                    if db::delete_record(&mut conn, id)? {
                        format!("Record #{id} deleted")
                    } else {
                        format!("Record #{id} no longer exists")
                    }
                }
                AdminAction::EndSession(id) => match db::end_session_admin(&mut conn, id)? {
                    Some(duration) => {
                        format!(
                            "Session #{id} ended ({})",
                ...
                AdminAction::DiscardSession(id) => {
                    if db::discard_session_admin(&mut conn, id)? {
...
pub fn run(pool: DbPool, options: TuiOptions) -> Result<()> {
    anyhow::ensure!(
        io::stdin().is_terminal() && io::stdout().is_terminal(),
        "the TUI requires an interactive terminal"
    );
    let mut app = App::new(pool, options)?;
...
mod tests {
    ...
    fn options(client_id: &str) -> TuiOptions {
        TuiOptions {
    ...
    fn app_loads_stats_records_sessions_and_tokens() {
        ...
        let app = App::new(pool, options("workstation")).unwrap();
    ...
    fn all_views_render_at_the_minimum_supported_size() {
        let pool = db::create_pool(":memory:").unwrap();
        let mut app = App::new(pool, options(GLOBAL_CLIENT)).unwrap();
